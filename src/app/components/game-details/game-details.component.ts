...
export class GameDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.localStorageService.ownedGames.subscribe(
      (games) => (this.ownedGames = _.mapKeys(games, 'id'))
    );
    this.localStorageService.wishListGames.subscribe(
      (games) => (this.wishListGames = _.mapKeys(games, 'id'))
    );
  }
}
